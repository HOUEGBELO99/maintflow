/**
 * Canonical demo dataset (Industries Nord, Site de Lyon), ported from the
 * prototype. Idempotent via upsert. Used by the CLI seed (`prisma/seed.ts`)
 * and by the dev-only "reset demo data" endpoint.
 *
 * Local dev only. In production, User.id MUST equal the Supabase auth UID.
 * IDs are deterministic, valid v4-format UUIDs (version nibble 4, variant
 * nibble a) so they also pass @IsUUID() when referenced as FKs.
 */
...
/** Resolve a prototype display-name to its user id (throws on a typo). */
function uid(name: string): string {
  const id = U[name];
  if (!id) throw new Error(`Unknown demo user: ${name}`);
  return id;
}

/** Lookup an id in a keyed demo list by a business code (throws if missing). */
function byCode<T extends { id: string }>(rows: readonly T[], code: string, get: (r: T) => string): string {
  const row = rows.find((r) => get(r) === code);
  if (!row) throw new Error(`Unknown demo code: ${code}`);
  return row.id;
}
...
export async function seedDemoData(prisma: PrismaClient): Promise<SeedCounts> {
  const site = await prisma.site.upsert({
    ...
    update: {},
    create: { id: DEMO_SITE_ID, name: 'Industries Nord — Site de Lyon', hourlyDowntimeCost: 850, currency: 'EUR' },
  });

  // ── Users (U1..U6) ─────────────────────────────────────────────────────────
  const users = [
    { id: uid('L. Moreau'), name: 'Laurent Moreau', email: 'l.moreau@usine.fr', role: 'admin', workshop: 'Tous', color: '#00C24A', initials: 'LM', lastLogin: '2026-05-21T08:14:00' },
    { id: uid('S. Diallo'), name: 'Sophie Diallo', email: 's.diallo@usine.fr', role: 'technicien', workshop: 'Atelier A', color: '#2563EB', initials: 'SD', lastLogin: '2026-05-21T07:32:00' },
    { id: uid('J. Petit'), name: 'Julien Petit', email: 'j.petit@usine.fr', role: 'technicien', workshop: 'Atelier B', color: '#F59E0B', initials: 'JP', lastLogin: '2026-05-20T16:55:00' },
    { id: uid('T. Khan'), name: 'Tarek Khan', email: 't.khan@usine.fr', role: 'operateur', workshop: 'Atelier B', color: '#7C3AED', initials: 'TK', lastLogin: '2026-05-21T06:10:00' },
  ...
  const invitations = [
    { id: '00000000-0000-4000-a000-000000000801', email: 'p.kone@usine.fr', role: 'technicien', workshop: 'Atelier C', by: uid('L. Moreau'), sentAt: '2026-05-20T10:00:00', status: 'pending' },
    { id: '00000000-0000-4000-a000-000000000802', email: 'n.bernard@usine.fr', role: 'operateur', workshop: 'Atelier D', by: uid('L. Moreau'), sentAt: '2026-05-18T15:30:00', status: 'pending' },
  ...
  const technicians = [
    { id: '00000000-0000-4000-a000-000000000501', userId: uid('L. Moreau'), title: 'Technicien sénior', specialties: ['mécanique', 'hydraulique'], available: true, onTime: 94, rating: 4.8, doneThisMonth: 14 },
    { id: '00000000-0000-4000-a000-000000000502', userId: uid('S. Diallo'), title: 'Technicien', specialties: ['électrique', 'mécanique'], available: true, onTime: 88, rating: 4.5, doneThisMonth: 11 },
    { id: '00000000-0000-4000-a000-000000000503', userId: uid('J. Petit'), title: 'Technicien', specialties: ['électrique', 'logiciel'], available: false, onTime: 91, rating: 4.6, doneThisMonth: 9 },
    { id: '00000000-0000-4000-a000-000000000504', userId: uid('T. Khan'), title: 'Technicien junior', specialties: ['hydraulique', 'mécanique'], available: true, onTime: 79, rating: 4.2, doneThisMonth: 7 },
  ] as const;
  ...
  const mid = (code: string): string => byCode(machines, code, (m) => m.code);
  ...
  const fid = (code: string): string => byCode(faults, code, (f) => f.code);
  for (const f of faults) {
    await prisma.fault.upsert({
      ...
      create: {
        ...
        severity: f.severity, status: f.status, rootCause: f.rootCause, reportedById: uid(f.by),
  ...
  const pid = (code: string): string => byCode(planRules, code, (p) => p.code);
  for (const p of planRules) {
    const data = {
      siteId: site.id, code: p.code, machineId: mid(p.machine), title: p.title, everyWeeks: p.everyWeeks,
      technicianId: uid(p.by), duration: p.duration, nextDue: new Date(p.nextDue), reminderLead: p.reminderLead, active: true,
    };
  ...
  for (const r of reminders) {
    const data = {
      siteId: site.id, ruleId: pid(r.rule), title: r.title, machineId: mid(r.machine), technicianId: uid(r.by),
  ...
  for (const i of interventions) {
    ...
    await prisma.intervention.upsert({
      ...
      update: {},
      create: {
        id: i.id, siteId: site.id, machineId: mid(i.machine), technicianId: uid(i.by), kind: i.kind,
